...
export const setQuery = (setQuery: string) => {
...
export const getSearchedMovies = () => (dispatch: any, getState: any) => {
  const state = getState();
  const queryParams = state.query;

  fetch(
    `https://api.themoviedb.org/3/search/movie?api_key=21e7b70aa3037a9a670477c21bc87838&language=en-US&query=${queryParams}&page=1&include_adult=false`
  )
    .then((res) => res.json())
    .then((data) => {
...
export const getTopRated = () => (dispatch: any) => {
  fetch(
